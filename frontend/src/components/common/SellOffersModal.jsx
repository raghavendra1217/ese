...
const SellOffersModal = ({ isOpen, onClose, onAcceptOffer, currentPrice, currentPrice2, currentPrice3, productName, stockCount }) => {
  ...
  const generateOffers = () => {
    ...
    // Ensure all prices are valid numbers, fallback to currentPrice if others are invalid
    const validPrices = [
      currentPrice || 0,
      currentPrice2 || currentPrice || 0,
      currentPrice3 || currentPrice || 0
    ].map(price => isNaN(price) ? (currentPrice || 0) : price);
    
    // Shuffle the 3 prices randomly
    const shuffledPrices = [...validPrices].sort(() => Math.random() - 0.5);
