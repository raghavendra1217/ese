...
// =================================================================
// --- COORDINATOR PRODUCT REQUEST FUNCTIONS ---
// =================================================================

// Submit product request (coordinator version)
exports.submitProductRequest = async (req, res) => {
    ...
    // Check if coordinator has pending request
    ...
    try {
        ...
        try {
            ...
            await runPy('../utils/sendAdminNotificationEmail.py', [
                'Coordinator Product Request Submitted',
                `Coordinator: ${userId}\nAmount: ₹${requestAmount.toFixed(2)}\nRemarks: ${remarks || 'No remarks provided'}`
            ]);
        } catch (e) {
            console.error('Admin email failed (coordinator product request):', e?.message || e);
        }
        ...
        console.error('❌ Error in coordinator submitProductRequest:', error);
...
// Get coordinator's current pending request
exports.getCurrentPendingRequest = async (req, res) => {
    ...
    try {
        ...
        console.error('❌ Error fetching coordinator pending request:', error);
        res.status(500).json({ message: 'Server error while fetching pending request.' });
    }
};

// Get coordinator's product request history
exports.getProductRequestHistory = async (req, res) => {
    ...
    try {
        ...
        console.error('❌ Error fetching coordinator product request history:', error);
        res.status(500).json({ message: 'Server error while fetching request history.' });
    }
};

// Cancel pending product request (coordinator version)
exports.cancelProductRequest = async (req, res) => {
    ...
    try {
        ...
        console.error('❌ Error cancelling coordinator product request:', error);
